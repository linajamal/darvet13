...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_total_line(self, options, account, amount_currency, debit, credit, balance):
        if 'curr' in self._context:
            cur = self.env['res.currency'].browse(self._context.get('curr'))
            columns = [
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id, cur, debit),
                                           currency=cur), 'class': 'number'},
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id, cur, credit),
                                           currency=cur), 'class': 'number'},
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id, cur, balance),
                                           currency=cur), 'class': 'number'},
            ]
    ...
    def _get_initial_balance_line(self, options, account, amount_currency, debit, credit, balance):
        if 'curr' in self._context:
            ...
            if self.user_has_groups('base.group_multi_currency'):
                columns.insert(0,{'name': self.format_value(amount_currency, currency=account.currency_id, blank_if_zero=True) or '', 'class': 'number'})
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        if 'curr' in self._context:
            ...
            columns = [
                ...
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id, cur, aml['debit']),
                                           currency=cur, blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id, cur, aml['credit']),
                                           currency=cur, blank_if_zero=True), 'class': 'number'},
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id, cur, cumulated_balance),
                                           currency=cur), 'class': 'number'},
            ]
            if self.user_has_groups('base.group_multi_currency'):

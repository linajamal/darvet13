...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_options(self, previous_options=None):
        ...
        _logger.info('IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII')
        _logger.info(res)
        if self.filter_currencys :
    ...
    def _get_report_line_partner(self, options, partner, initial_balance, debit, credit, balance):
        if 'curr' in self._context:
            ...
            columns = [
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id,cur,initial_balance),currency=cur), 'class': 'number'},
            ...
            if self.user_has_groups('base.group_multi_currency'):
                columns.append({'name': ''})
            columns.append({'name': self.format_value(cur._compute(self.env.user.company_id.currency_id,cur,balance),currency=cur), 'class': 'number'})
        ...
        _logger.info('IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII 1414444444')
    ...
    def _get_report_line_move_line(self, options, partner, aml, cumulated_init_balance, cumulated_balance):
        if 'curr' in self._context:
            ...
            columns = [
                ...
                {'name': self.format_value(cur._compute(self.env.user.company_id.currency_id,cur,cumulated_init_balance),currency=cur), 'class': 'number'},
            ...
            if self.user_has_groups('base.group_multi_currency'):
                if aml['currency_id']:
                    ...
                    columns.append({'name': ''})
            columns.append({'name': self.format_value(cur._compute(self.env.user.company_id.currency_id,cur,cumulated_balance),currency=cur), 'class': 'number'})
            return {
                'id': aml['id'],
